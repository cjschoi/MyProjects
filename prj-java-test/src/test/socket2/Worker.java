...
public class Worker implements Callable<Void> {
	...
	public Void call() throws Exception {
		...
		StringBuffer stringBuffer = new StringBuffer(new String(b, 0, byteBuffer.limit()));
		...
		if (byteBuffer.capacity() < returnData.length) {
			...
			System.out.println("****INCREASING SIZE " + byteBuffer.position() + " " + byteBuffer.limit());
		} else {
			byteBuffer.clear();
			byteBuffer.put(returnData);
			byteBuffer.flip();
			System.out.println("****NOT INCREASING SIZE " + byteBuffer.position() + " " + byteBuffer.limit());
		}

		Server.byteBufferStore.put(key, byteBuffer);
		System.out.println("WORKER COMPLETED '" + new String(byteBuffer.array(), 0, byteBuffer.limit()) + "' "
